...
class M3AETransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.hparams.config["load_path"] != "" and not self.hparams.config["test_only"]:
            ...
            #1031
            original_layer = self.vqa_head[3]  # Save the original layer
            self.vqa_head[3] = nn.Linear(1536, 498)  # Set to checkpoint size for loading    
            self.load_state_dict(state_dict, strict=False)
            self.vqa_head[3] = nn.Linear(1536, 700)  # Reset to desired size
            self.vqa_head[3].to(self.device)
            nn.init.xavier_uniform_(self.vqa_head[3].weight)
            nn.init.zeros_(self.vqa_head[3].bias)
        # == End  : 3. Load Models ==
